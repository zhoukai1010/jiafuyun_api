...
class Malsapi {
    ...
    protected function __construct() {
        ...
        $this->parser = new MalsapiParserJson();
    }
     ...
     * @param MalsapiFilter $filter 过滤器
     * @return Malsapi
     */
    public function withFilter(MalsapiFilter $filter) {
        $this->filter = $filter;
        return $this;
    }
     ...
     * @param MalsapiParser $parser 结果解析器
     * @return Malsapi
     */
    public function withParser(MalsapiParser $parser) {
        $this->parser = $parser;
        return $this;
    }
 ...
 * JSON解析
 */
class MalsapiParserJson implements MalsapiParser {

    public function parse($apiResult) {
        if ($apiResult === false) {
            return new MalsapiResponse(408, array(), 'Request Timeout');
        }

        $arr = json_decode($apiResult, true);

        if ($arr === false || empty($arr)) {
            return new MalsapiResponse(500, array(), 'Internal Server Error');
        }

        return new MalsapiResponse($arr['ret'], $arr['data'], $arr['msg']);
    }
}
