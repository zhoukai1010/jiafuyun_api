...
class Api_Notify extends PhalApi_Api {
    ...
    public function changeOrderStatus($info){

        //更新客户订单
        $payDomain = new Domain_Pay();
        $data = array();
        $data['item_bn'] = $info['out_trade_no'];
        $data['pay_type'] = $this->type;
        $data['serial_bn'] = $info['trade_no'];
        $data['openid'] = $info['openid'];
        $data['status'] = 'finish';
        $data['pay_status'] = 'succ';
        $data['pay_time'] = strtotime($info['time_end']);
        $data['last_modify'] = time();

        $payDomain->payOk($data);
