...
class Domain_Sms {
    ...
    public function  sendCode($mobile){
        $smsCode = new Model_SmsCheckCode ();
        $data = array (
            'mobile' => $mobile,
        ...
        if (! $id) {
            ...
            $this->sendSms('',$mobile, $content, '6');
        }
        return $id;
    }
    //地阳短信
    public function sendSms($nationCode, $mobile, $content, $productCode) {
        switch ($productCode){
        ...
        if($errormsg == ''){
            //更新发送日志/短信余额
            $domainSmsLog = new Domain_PartnerSmsSendLog();
            $domainSmsLog->addSendLog($mobile,$content);
            return $result;
        }else{
    ...
    function checkCode($mobile,$smsCode){
        ...
        if($check){
            if($check['create_time'] > (time()-$this->smsVaildTime)){
                ...
                throw new LogicException ( T ( 'Sms code error' ), 109 );
            }
        }else{
            throw new LogicException ( T ( 'Sms code error' ), 109 );
        }
