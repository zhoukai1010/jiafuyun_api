...
//    );
//}
function sp_request($service,$params = array(),$timeout = 3000){
    $api_url  =  DI ()->config->get ( 'app.shenpu.get_api_url' );
    $client = Malsapi::create()->withHost($api_url);
    $client->reset()->withService($service);
    if ($params) {
...
function jfb_request($service,$params = array(),$timeout = 3000){
    $api_url  =  DI ()->config->get ( 'app.jfb.get_api_url' );
    $client = Malsapi::create()->withHost($api_url);
    $client->reset()->withService($service);
    if ($params) {
        foreach ($params as $k => $v) {
...
if($debug){
    ...
    DI()->cookie = new PhalApi_Cookie(array('domain'=>'.jiafuyun.com','path'=>'/'));
}
