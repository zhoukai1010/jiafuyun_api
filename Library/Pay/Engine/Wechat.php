...
class Pay_Engine_Wechat extends Pay_Base {
	...
	protected $query_url = "https://api.mch.weixin.qq.com/pay/orderquery?";
	protected $refund_url = "https://api.mch.weixin.qq.com/secapi/pay/refund?";
	protected $transfers_url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers?";
	...
	public function queryOrder($orderBn){
		...
		$response = $this->postXmlCurl($xml, $this->query_url, false, 6);
		$this->values = $this->xmlToArray($response);
		if($this->values['return_code'] != 'SUCCESS'){
			DI()->logger->log('queryOrderError','查询失败', $this->values);
			return false;
		}
		if($this->values['return_code'] == 'SUCCESS'){
			return $this->values;
     ...
     * 申请退款
     */
    public function applyRefund($orderBn,$refundBn,$total_fee,$refund_fee){
        $this->param['appid'] = $this->config['appid'];
        $this->param['mch_id'] = $this->config['mchid'];
        $this->param['out_refund_no'] = $refundBn;//退款单号
        $this->param['total_fee'] = $total_fee;//订单金额
        $this->param['refund_fee'] = $refund_fee;//退款金额
        $this->param['op_user_id'] = $this->config['mchid'];//操作员ID 默认为商户号
        $this->param['out_trade_no'] = $orderBn;
        $this->param['nonce_str'] = $this->createNoncestr(32);
        //获取签名信息
        $this->param['sign'] = $this->getSign($this->param);
        ...
        //提交XML信息后，将返回的XML转换成数组
        $response = $this->curl_post_ssl( $this->refund_url, $xml);
        $this->values = $this->xmlToArray($response);

        if($this->values['return_code'] != 'SUCCESS'){
            DI()->logger->log('refundError','退款失败', $this->values);
            return false;
        }
        if($this->values['return_code'] == 'SUCCESS'){
            return $this->values;
     ...
     * 企业付款给个人用户
     */
    public function transfers($orderBn,$openId,$checkName,$name='',$amount,$desc){
        $this->param['mch_appid'] = $this->config['appid'];
        $this->param['mchid'] = $this->config['mchid'];
        $this->param['openid'] = $openId;
        if($checkName == 'y'){
            $this->param['check_name'] = 'FORCE_CHECK';
            $this->param['re_user_name'] = $name;
        }else{
            $this->param['check_name'] = 'NO_CHECK';
        }
        $this->param['amount'] = $amount;
        if($desc) {
            $this->param['desc'] = $desc;
        }else{
            $this->param['desc'] = '客户打款';
        }
        $this->param['spbill_create_ip'] = PhalApi_Tool::getClientIp();
        $this->param['partner_trade_no'] = $orderBn;
        $this->param['nonce_str'] = $this->createNoncestr(32);
        //获取签名信息
        $this->param['sign'] = $this->getSign($this->param);
        $xml = $this->arrayToXml($this->param);
        ...
        //提交XML信息后，将返回的XML转换成数组
        $response = $this->curl_post_ssl( $this->transfers_url, $xml);
        $this->values = $this->xmlToArray($response);
        if($this->values['return_code'] != 'SUCCESS'){
            DI()->logger->log('transfersError','付款失败', $this->values);
            return false;
        }
        if($this->values['return_code'] == 'SUCCESS'){
            return $this->values;
    ...
    protected function setInfo($notify) {
		...
		//支付时间
		$info['time_end'] = $notify['time_end'];
		//商户号
		$info['mch_id'] = $notify['mch_id'];
		//支付者openid
		$info['openid'] = $notify['openid'];
        $this->info = $info;
    }
