...
abstract class Wechat_Robot {
    ...
    public function run()
    {
        ...
        // 未通过消息真假性验证
        if (!$this->debug && !$this->validateSignature($mpToken) ) {
        ...
        try{
            ...
            if($inMessage->isTestCase()){
                $pluginInstance = new Plugin_Wechat_TestCase();
            }else{
                $pluginName = sprintf('Plugin_Wechat_%s',ucfirst($inMessage->getMsgType()));
